...
if (!isset($_SESSION['role']) || ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'member')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_SESSION['role'] === 'admin') {
    ...
    $update_stmt = $conn->prepare("UPDATE contributions SET status = 'paid', amount = ?, payment_method = ? WHERE id = ?");
    $update_stmt->bind_param("dsi", $amount, $payment_method, $contrib_id);
    $update_stmt->execute();
    $update_stmt->close();
...
if (!isset($_GET['death_id'])) {
    $result = $conn->query("SELECT id, deceased_name FROM deaths ORDER BY created_at DESC");
    ?>
    <!DOCTYPE html>
    <html>
    <head>
...
<?php if (!empty($affected_member_name)): ?>
    <h4>Reported By Member: <?php echo htmlspecialchars($affected_member_name); ?> | ID: <?php echo htmlspecialchars($affected_id_number); ?> | Phone: <?php echo htmlspecialchars($affected_phone); ?></h4>
<?php endif; ?>
...
$total_stmt = $conn->prepare("SELECT SUM(amount) AS total_amount FROM contributions WHERE death_id = ?");
...
<h3>Total Contributions Collected: KES <?php echo number_format($total_amount, 2); ?></h3>
